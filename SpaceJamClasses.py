...
class Wanderer(SphereCollideObject):
    ...
    def __init__(self, loader, modelPath, parentNode, nodeName, scaleVec, texPath, staringAt, colPositionVec, colRadius):
...
class Orbiter(SphereCollideObject):
    ...
    def __init__(self, loader, taskMgr, modelPath: str, parentNode: NodePath, nodeName: str, 
        ...
        self.cloudClock = 0
        self.orbitClock = 0  # <-- FIXED: Initialize orbitClock to avoid AttributeError
    ...
    def Orbit(self, task):
        if self.orbitType == "MLB":
            self.orbitClock += 1
            positionVec = defensePaths.BaseballSeams(self.orbitClock, 60, B=0.4)
            newPos = positionVec * self.orbitRadius + self.orbitObject.getPos()
        ...
        # Only attempt to look at something if staringAt is valid
        if self.staringAt is not None and hasattr(self.staringAt, 'modelNode'):
