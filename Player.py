...
class Spaceship(SphereCollideObject):
    def __init__(self, loader, accept, modelPath, parentNode, nodeName, texPath, posVec, scaleVec, taskMgr, camera):
        ...
        self.boostSpeed = 15  # Normal forward speed is 5, boost is 3x faster
        self.boostDuration = 10.0  # Boost lasts 10 seconds
        self.boostCooldownTime = 5.0  # Cooldown lasts 5 seconds
        self.boostMeter = OnscreenText(text="Boost: Ready", pos=(-1, 0.8), scale=0.07, fg=(0, 1, 0, 1))
    ...
    def SetKeyBindings(self):
        ...
        self.accept('b', self.ActivateBoost)
    ...
    def ActivateBoost(self):
        if not self.boostActive and not self.boostCooldown:
            self.boostActive = True
            self.boostMeter.setText("Boost: Active")
            self.boostMeter.setFg((1, 0.5, 0, 1))  # Orange when active
            self.taskMgr.doMethodLater(self.boostDuration, self.EndBoost, 'boostDuration')
        print("Boost activated!")

    def EndBoost(self, task):
        self.boostActive = False
        self.boostCooldown = True
        self.boostMeter.setText("Boost: Recharging")
        self.boostMeter.setFg((1, 0, 0, 1))  # Red when recharging
        self.taskMgr.doMethodLater(self.boostCooldownTime, self.ResetBoost, 'boostCooldown')
        print("Boost ended, recharging...")
        return Task.done

    def ResetBoost(self, task):
        self.boostCooldown = False
        self.boostMeter.setText("Boost: Ready")
        self.boostMeter.setFg((0, 1, 0, 1))  # Green when ready
    ...
    def ApplyMoveForward(self, task):
        rate = self.boostSpeed if self.boostActive else 5
